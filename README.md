...
The root cause turned out to be geometric: since MinMax-scaled values 
are all non-negative, every song's feature vector was confined to a 
single positive region of the space, which keeps the angle between any 
two vectors artificially small regardless of how different the songs 
actually are. PCA cannot fix this, since it operates on variance, not 
